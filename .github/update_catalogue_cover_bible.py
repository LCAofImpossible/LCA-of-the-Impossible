...
start = text.index('## 4. Episode asset convention')
end = text.index('\n---\n\n## 5. Mandatory editorial structure of every episode page', start)
new_section = '''## 4. Episode asset convention
...
old13 = '''## 13. Catalogue-cover rules — mandatory
...
new13 = '''## 13. Catalogue-cover rules — mandatory
...
assert old13 in text, 'Section 13 block not found'
text = text.replace(old13, new13)

old_cover_qa = '''### Catalogue cover — homepage/archive only

- [ ] Cover matches page 1 or approved replacement.
- [ ] No unauthorized redraw/restyle.
- [ ] Cover renders in homepage/archive.'''
new_cover_qa = '''### Catalogue cover — homepage/archive only
...
assert old_cover_qa in text, 'Cover QA checklist block not found'
text = text.replace(old_cover_qa, new_cover_qa)

old_workflow = '''1. Read approved episode PDF completely.
...
new_workflow = '''1. Read approved episode PDF completely.
2. Extract title, number, series, FU, result, hotspot, inventory, exclusions, sensitivities and verdict.
...
5. Create Inventory Map SVG.'''
assert old_workflow in text, 'Publishing workflow cover steps not found'
...
text = text.replace(
    '**The approved PDF controls episode-specific facts, numbers, wording and approved cover artwork.**',
    '**The approved PDF controls episode-specific facts, numbers and wording; the website catalogue cover follows the canonical visual family unless an exact cover is explicitly approved by the user.**'
)
text = text.replace(
    '**Approved covers are catalogue assets displayed only on homepage and Archive.**',
    '**Approved catalogue covers are 4:5 assets in the canonical #35/#36/#40/#41/#42/#43 visual family and are displayed only on Homepage and Archive.**'
)

oldqa = '''### Catalogue-cover QA
...
- When replacing a live cover, use a cache-safe publication path and confirm the new asset is what GitHub Pages serves.'''
assert oldqa in text, 'Final catalogue-cover QA block not found'
text = text.replace(oldqa, newqa)
