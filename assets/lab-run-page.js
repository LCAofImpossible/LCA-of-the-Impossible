(() => {
  ...
  const render = (message = '') => {
    ...
    if (status) {
      if (message) {
        ...
        status.textContent = 'Browser storage is unavailable. The six-stage Run cannot continue between pages on this device.';
  ...
  const start = (restart = false) => {
    if (restart && !window.confirm('Restart the current Impossible Lab Run? Its six-stage progress will be replaced.')) return;
