(() => {
  ...
  const render = (message = '') => {
    ...
    if (status) {
      if (message) {
        ...
        status.textContent = `Browser storage is unavailable. The ${runSystem.GAME_IDS.length}-stage Run cannot continue between pages on this device.`;
  ...
  const start = (restart = false) => {
    if (restart && !window.confirm(`Restart the current Impossible Lab Run? Its ${runSystem.GAME_IDS.length}-stage progress will be replaced.`)) return;
